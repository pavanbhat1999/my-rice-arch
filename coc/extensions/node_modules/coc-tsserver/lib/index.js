...
function convertDocumentHighlight(highlight) {
  return highlight.highlightSpans.map((span) => {
    return {
      ...
      kind: span.kind === "writtenReference" ? import_vscode_languageserver_protocol9.DocumentHighlightKind.Write : import_vscode_languageserver_protocol9.DocumentHighlightKind.Read
    };
  });
}
...
var getSymbolKind2 = (kind) => {
  switch (kind) {
    case Kind.module:
      return import_vscode_languageserver_protocol10.SymbolKind.Module;
    case Kind.class:
      return import_vscode_languageserver_protocol10.SymbolKind.Class;
    case Kind.enum:
      return import_vscode_languageserver_protocol10.SymbolKind.Enum;
    case Kind.interface:
      return import_vscode_languageserver_protocol10.SymbolKind.Interface;
    case Kind.method:
      return import_vscode_languageserver_protocol10.SymbolKind.Method;
    case Kind.memberVariable:
      return import_vscode_languageserver_protocol10.SymbolKind.Property;
    case Kind.memberGetAccessor:
      return import_vscode_languageserver_protocol10.SymbolKind.Property;
    case Kind.memberSetAccessor:
      return import_vscode_languageserver_protocol10.SymbolKind.Property;
    case Kind.variable:
      return import_vscode_languageserver_protocol10.SymbolKind.Variable;
    case Kind.const:
      return import_vscode_languageserver_protocol10.SymbolKind.Variable;
    case Kind.localVariable:
      return import_vscode_languageserver_protocol10.SymbolKind.Variable;
    ...
    case Kind.function:
    case Kind.localFunction:
      return import_vscode_languageserver_protocol10.SymbolKind.Function;
  }
  return import_vscode_languageserver_protocol10.SymbolKind.Variable;
};
var TypeScriptDocumentSymbolProvider = class {
  ...
  static convertSymbol(item, range) {
    ...
    const symbolInfo = import_vscode_languageserver_protocol10.DocumentSymbol.create(label, "", getSymbolKind2(item.kind), range, containsRange(range, selectionRange) ? selectionRange : range);
    const kindModifiers = parseKindModifier2(item.kindModifiers);
    if (kindModifiers.has(KindModifiers.deprecated)) {
      symbolInfo.tags = [import_vscode_languageserver_protocol10.SymbolTag.Deprecated];
    }
    return symbolInfo;
  }
  ...
  async provideCodeActions(document, _range, context, _token) {
    ...
    for (const diagnostic of diagnostics) {
      ...
      if (nodeModules.indexOf(name) !== -1) {
        names.push(name);
        edits.push({
          range: import_vscode_languageserver_protocol11.Range.create(0, 0, 0, 0),
...
var InstallModuleProvider = class {
  ...
  async provideCodeActions(document, _range, context, _token) {
    ...
    for (let name of names) {
      ...
      let codeAction = import_vscode_languageserver_protocol12.CodeAction.create(title, command, import_vscode_languageserver_protocol12.CodeActionKind.QuickFix);
...
var ApplyFixAllCodeAction2 = class {
  ...
  async execute(document, file2, tsAction) {
    ...
    await this.formattingConfigurationManager.ensureConfigurationForDocument(document, import_vscode_languageserver_protocol13.CancellationToken.None);
    const args = {
      scope: {
        type: "file",
        args: {file: file2}
    ...
    try {
      const res = await this.client.execute("getCombinedCodeFix", args, import_vscode_languageserver_protocol13.CancellationToken.None);
      if (res.type != "response") {
        return;
      }
      let {body} = res;
      ...
      const token = import_vscode_languageserver_protocol13.CancellationToken.None;
...
var SupportedCodeActionProvider = class {
  ...
  get supportedCodeActions() {
    if (!this._supportedCodeActions) {
      return new Promise((resolve, reject) => {
        this.client.execute("getSupportedCodeFixes", null, import_vscode_languageserver_protocol13.CancellationToken.None).then((res) => {
          if (res.type !== "response") {
            resolve(new Set());
            return;
          }
...
var TypeScriptQuickFixProvider = class {
  ...
  getSingleFixForTsCodeAction(diagnostic, tsAction) {
    const codeAction = {
      ...
      kind: import_vscode_languageserver_protocol13.CodeActionKind.QuickFix
    };
...
var ApplyRefactoringCommand2 = class {
  ...
  async execute(document, file2, refactor, action, range) {
    ...
    const response = await this.client.execute("getEditsForRefactor", args, import_vscode_languageserver_protocol14.CancellationToken.None);
...
var TypeScriptRefactorProvider2 = class {
  ...
  async provideCodeActions(document, range, context, token) {
    ...
    return this.convertApplicableRefactors(response.body, document, file2, range, context.only && context.only.some((v) => v.includes(import_vscode_languageserver_protocol14.CodeActionKind.Refactor)));
  }
    ...
    for (const info of body) {
      ...
      if (!info.inlineable) {
        const codeAction = {
          ...
          kind: import_vscode_languageserver_protocol14.CodeActionKind.Refactor
        };
        codeAction.command = {
          title: info.description,
  ...
  shouldTrigger(context) {
    if (context.only && context.only.every((o) => !o.includes(import_vscode_languageserver_protocol14.CodeActionKind.Refactor))) {
      return false;
    }
    return true;
  }
  static getKind(refactor) {
    ...
    return import_vscode_languageserver_protocol14.CodeActionKind.Refactor;
  }
...
TypeScriptRefactorProvider.extractFunctionKind = import_vscode_languageserver_protocol14.CodeActionKind.RefactorExtract + ".function";
TypeScriptRefactorProvider.extractConstantKind = import_vscode_languageserver_protocol14.CodeActionKind.RefactorExtract + ".constant";
TypeScriptRefactorProvider.moveKind = import_vscode_languageserver_protocol14.CodeActionKind.Refactor + ".move";
TypeScriptRefactorProvider.metadata = {
  providedCodeActionKinds: [import_vscode_languageserver_protocol14.CodeActionKind.Refactor]
};
...
var UpdateImportsOnFileRenameHandler = class {
  ...
  async getEditsForFileRename(document, oldFile, newFile) {
    await this.fileConfigurationManager.ensureConfigurationForDocument(document, import_vscode_languageserver_protocol15.CancellationToken.None);
    const response = await this.client.interruptGetErr(() => {
      const args = {
        oldFilePath: oldFile,
        newFilePath: newFile
      };
      return this.client.execute("getEditsForFileRename", args, import_vscode_languageserver_protocol15.CancellationToken.None);
    });
    if (!response || response.type != "response" || !response.body) {
      return;
    }
...
var OrganizeImportsCommand = class {
  ...
  async _execute(client, document) {
    ...
    const response = await client.interruptGetErr(() => client.execute("organizeImports", args, import_vscode_languageserver_protocol16.CancellationToken.None));
...
var OrganizeImportsCodeActionProvider = class {
  constructor(client, fileConfigManager) {
    ...
    this.metadata = {
      providedCodeActionKinds: [import_vscode_languageserver_protocol16.CodeActionKind.SourceOrganizeImports]
    };
  }
  async provideCodeActions(document, _range, context, token) {
    if (this.client.modeIds.indexOf(document.languageId) == -1)
      return;
    if (!context.only || !context.only.includes(import_vscode_languageserver_protocol16.CodeActionKind.SourceOrganizeImports)) {
      return [];
    }
    await this.fileConfigManager.ensureConfigurationForDocument(document, token);
    const action = import_vscode_languageserver_protocol16.CodeAction.create("Organize Imports", {
      title: "",
      command: "tsserver.organizeImports",
      arguments: [document]
    }, import_vscode_languageserver_protocol16.CodeActionKind.SourceOrganizeImports);
    return [action];
  }
};
...
var LanguageProvider = class {
  ...
  registerProviders(client, typingsStatus) {
    ...
    if (this.client.apiVersion.gte(api_default.v280)) {
      ...
      this._register(import_coc23.languages.registerCodeActionProvider(languageIds, new OrganizeImportsCodeActionProvider(this.client, this.fileConfigurationManager), "tsserver", [import_vscode_languageserver_protocol17.CodeActionKind.SourceOrganizeImports]));
    }
    if (this.client.apiVersion.gte(api_default.v240)) {
      this._register(import_coc23.languages.registerCodeActionProvider(languageIds, new refactor_default(client, this.fileConfigurationManager), "tsserver", [import_vscode_languageserver_protocol17.CodeActionKind.Refactor]));
    }
    this._register(import_coc23.languages.registerCodeActionProvider(languageIds, new quickfix_default(client, this.fileConfigurationManager), "tsserver", [import_vscode_languageserver_protocol17.CodeActionKind.QuickFix]));
    this._register(import_coc23.languages.registerCodeActionProvider(languageIds, new importFix_default(this.client.bufferSyncSupport), "tsserver", [import_vscode_languageserver_protocol17.CodeActionKind.QuickFix]));
...
var TypeScriptServiceClient = class {
  constructor(pluginManager, modeIds) {
    ...
    this._onTsServerStarted = new import_vscode_languageserver_protocol19.Emitter();
    this._onProjectLanguageServiceStateChanged = new import_vscode_languageserver_protocol19.Emitter();
    this._onDidBeginInstallTypings = new import_vscode_languageserver_protocol19.Emitter();
    this._onDidEndInstallTypings = new import_vscode_languageserver_protocol19.Emitter();
    this._onTypesInstallerInitializationFailed = new import_vscode_languageserver_protocol19.Emitter();
    this.disposables = [];
    this.isRestarting = false;
    this._onDiagnosticsReceived = new import_vscode_languageserver_protocol19.Emitter();
    this._onConfigDiagnosticsReceived = new import_vscode_languageserver_protocol19.Emitter();
    this._onResendModelsRequested = new import_vscode_languageserver_protocol19.Emitter();
  ...
  execute(command, args, token, config) {
    ...
    if (config == null ? void 0 : config.cancelOnResourceChange) {
      const source = new import_vscode_languageserver_protocol19.CancellationTokenSource();
...
var AtaProgressReporter = class {
  constructor(client) {
    ...
    this._disposable = import_vscode_languageserver_protocol20.Disposable.create(() => {
...
var TypeScriptServiceClientHost = class {
      ...
      if (body) {
        ...
        if (diagnostics.length == 0) {
          ...
          let diagnosticList = diagnostics.map((o) => {
            ...
            let range;
            if (!start || !end) {
              range = import_vscode_languageserver_protocol21.Range.create(import_vscode_languageserver_protocol21.Position.create(0, 0), import_vscode_languageserver_protocol21.Position.create(0, 1));
            } else {
              range = import_vscode_languageserver_protocol21.Range.create(start.line - 1, start.offset - 1, end.line - 1, end.offset - 1);
            }
            let severity = category == "error" ? import_vscode_languageserver_protocol21.DiagnosticSeverity.Error : import_vscode_languageserver_protocol21.DiagnosticSeverity.Warning;
            return import_vscode_languageserver_protocol21.Diagnostic.create(range, text, severity, code);
          });
  ...
  reloadProjects() {
    ...
    this.client.execute("reloadProjects", null, import_vscode_languageserver_protocol21.CancellationToken.None);
    this.triggerAllDiagnostics();
  }
  ...
  getDiagnosticSeverity(diagnostic) {
    if (this.reportStyleCheckAsWarnings && this.isStyleCheckDiagnostic(diagnostic.code) && diagnostic.category === DiagnosticCategory.error) {
      return import_vscode_languageserver_protocol21.DiagnosticSeverity.Warning;
    }
    switch (diagnostic.category) {
      case DiagnosticCategory.error:
        return import_vscode_languageserver_protocol21.DiagnosticSeverity.Error;
      case DiagnosticCategory.warning:
        return import_vscode_languageserver_protocol21.DiagnosticSeverity.Warning;
      case DiagnosticCategory.suggestion:
        return import_vscode_languageserver_protocol21.DiagnosticSeverity.Information;
      default:
        return import_vscode_languageserver_protocol21.DiagnosticSeverity.Error;
    }
  }
...
var TsserverService = class {
  constructor(pluginManager) {
    ...
    this._onDidServiceReady = new import_vscode_languageserver_protocol22.Emitter();
...
var PluginManager = class {
  constructor() {
    ...
    this._disposables = [];
    this._onDidUpdatePlugins = this._register(new import_vscode_languageserver_protocol23.Emitter());
    this.onDidChangePlugins = this._onDidUpdatePlugins.event;
    this._onDidUpdateConfig = this._register(new import_vscode_languageserver_protocol23.Emitter());
